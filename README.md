...
원본 스크린샷처럼 낮은 해상도로 뭉개진 한국어 흑백 비트맵 폰트입니다. `가`부터 `힣`까지 완성형 한글 11,172자를 실제 한국어 폰트에서 작게 래스터라이즈한 뒤, 작은 크기에서도 읽히도록 8x9 흑백 픽셀로 저장합니다.

## 구성

- `dist/THFOriginal.ttf`: 설치하거나 `@font-face`로 사용할 수 있는 TrueType 폰트 파일
- `font/thf-original.json`: Canvas 렌더러용 원본 스타일 흑백 픽셀 글리프 데이터
...
이 폰트는 인쇄용 폰트보다 레트로 UI, 게임 HUD, 저해상도 디스플레이, 글리치 그래픽에 어울립니다. 각 글리프는 Windows의 Malgun Gothic Bold를 아주 작게 래스터라이즈한 뒤 8x9 흑백 픽셀로 임계처리해 생성합니다.
