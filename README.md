...
픽셀화된 3x9 그레이스케일 한국어 비트맵 폰트입니다. `가`부터 `힣`까지 완성형 한글 11,172자를 모두 3x9 픽셀 셀 하나씩 따로 저장합니다.

## 구성

- `font/thf-3x9-gray.json`: 3x9 픽셀 자모/ASCII 글리프 데이터
- `scripts/generate-hangul.mjs`: 완성형 한글 전체 글리프 생성기
- `src/thf3x9.js`: Canvas 렌더러
...
한글 완성형 글자는 모두 개별 글리프로 저장되어 렌더링됩니다. 폰트 데이터에 없는 문자는 `?`로 대체하지 않고, 해당 코드포인트에서 파생된 3x9 유니코드 박스 글리프로 표시합니다.
